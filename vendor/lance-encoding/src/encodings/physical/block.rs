...
mod zstd {
    ...
    impl BufferCompressor for ZstdBufferCompressor {
        fn compress(&self, _input_buf: &[u8], _output_buf: &mut Vec<u8>) -> Result<()> {
            Err(Error::not_supported(
                "zstd compression is not supported in the wasm build",
            ))
        }
        ...
        fn decompress(&self, input_buf: &[u8], output_buf: &mut Vec<u8>) -> Result<()> {
            ...
            let mut decoder = ::zstd::decoding::StreamingDecoder::new(&mut cursor)
                .map_err(|err| Error::internal(format!("Zstd decompression error: {err}")))?;
            decoder
...
mod lz4 {
    ...
    impl BufferCompressor for Lz4BufferCompressor {
        fn compress(&self, input_buf: &[u8], output_buf: &mut Vec<u8>) -> Result<()> {
            output_buf.extend_from_slice(&::lz4::block::compress_prepend_size(input_buf));
            Ok(())
        }

        fn decompress(&self, input_buf: &[u8], output_buf: &mut Vec<u8>) -> Result<()> {
            let decompressed = ::lz4::block::decompress_size_prepended(input_buf)
                .map_err(|err| Error::internal(format!("LZ4 decompression error: {err}")))?;
